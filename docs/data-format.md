...
- `standardEdition` — optional edition or publication year used for review.
- `jurisdiction` — optional uppercase ISO country codes where the national
  collection applies.
- `references` — optional public provenance records with a title and optional
  identifier or HTTP(S) URL. References support review; content is still drawn
  independently.
- `review` — optional verification state: `unreviewed`, `technical-reviewed`
  or `market-verified`. Market verification also requires `verifiedAt` and at
  least one entry in `verifiedBy`. Missing reviewmetadata makes no implicit
  verification claim.
...
Every downloadable file appears in the collection entry's `files` array. The
same entry contains an `integrity` object with one value per file:

```json
{
  "files": ["symbols/smoke-detector.svg"],
  "integrity": {
    "symbols/smoke-detector.svg": "sha256-0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"
  }
}
```

Consumers can hash the downloaded bytes with SHA-256 and compare the lowercase
hex digest before caching or importing content. Collection `license`, optional
`description`, provenance and reviewmetadata are also copied into the index.
Run `npm run check-index` to validate both generated bytes and the public
`schema/index.schema.json` contract.
